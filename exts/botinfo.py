...
class CommandSource(menus.ListPageSource):
    ...
    async def format_page(self, menu, cmds):
        ...
        yee = "".join(cmds)
        cur_page = (
            f"Command Usage | Page {menu.current_page + 1}/{self.get_max_pages()}"
        )
        return ctx.embed(
            title=cur_page,
...
class BotInfo(commands.Cog):
    ...
    async def invite(self, ctx):
        ...
        invite = ctx.embed(
            title="Invite me to your server:", description=self.bot.invite
        )
        invite.add_field(
            name="You can also join the support server:", value=self.bot.support_invite
        )
    ...
    async def support(self, ctx):
        ...
        await ctx.send(
            embed=ctx.embed(
                ...
                description="https://discord.gg/NTNgvHkjSp",
            )
        )
    ...
    async def uptime(self, ctx):
        ...
        uptime = humanize.precisedelta(
            self.bot.start_time - datetime.datetime.utcnow(), format="%0.0f"
        )
    ...
    async def info(self, ctx):
        ...
        average_members = sum([guild.member_count for guild in self.bot.guilds]) / len(
            self.bot.guilds
        )
        ...
        emb.add_field(
            name="Command Count",
            value=f"```{len(set(self.bot.walk_commands())) - 31} entry```",
            inline=True,
        )
        emb.add_field(
            name="Guild Count",
        ...
        emb.add_field(
            name="Average Member Count",
            value=f"```{average_members:.0f} members per guild```",
        )
