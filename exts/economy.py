...
class Economy(commands.Cog):
    ...
    async def unregister(self, ctx):
        ...
        answer, message = await ctx.confirm(
            "React to confirm before doing this. This action is irreversible."
        )
        if answer:
            query = """
            ...
            return await message.edit(
                content="Successfully unregistered you on this server."
            )
    ...
    async def balance(self, ctx, user: discord.Member = None):
        ...
        e = ctx.embed(
            ...
            description=f"💸 **Cash:** ${humanize.intcomma(cash)}\n"
    ...
    @commands.command(
        aliases=("lb", "top", "server-leaderboard", "server-lb"),
        usage="[page=1] [--cash | --bank]",
    )
    async def leaderboard(self, ctx, page: typing.Optional[int] = 1, item: str = None):
        ...
        for user in data:
            ...
            name = discord.utils.escape_markdown(
                str(await self.bot.try_user(user["user_id"]))
            )
        ...
        embed = ctx.embed(
            title=f"{ctx.guild.name} Leaderboard", description="\n".join(lb)
        )
    ...
    async def deposit(self, ctx, amount: str):
        ...
        await ctx.send(
            embed=ctx.embed(
                description=f"You deposited **${humanize.intcomma(amount)}** into your bank."
            )
        )

    @commands.command(
        help="Withdraws a set amount from your bank", aliases=["wd", "with"]
    )
    async def withdraw(self, ctx, amount: str):
        ...
        await ctx.send(
            embed=ctx.embed(
                description=f"You withdrew **${humanize.intcomma(amount)}** from your bank."
            )
        )

    @commands.command(
        help="Lets you send money over to another user", aliases=("send", "pay", "give")
    )
    async def transfer(self, ctx, user: discord.Member, amount: str):
        ...
        async with self.bot.db.acquire() as conn:
            async with conn.transaction():
                await self.bot.db.execute(
                    "UPDATE economy SET cash = cash - $1 WHERE guild_id = $2 AND user_id = $3",
                    amount,
                    ctx.guild.id,
                    ctx.author.id,
                )
        ...
        await ctx.send(
            embed=ctx.embed(
                description=f"You gave {user.mention} **${humanize.intcomma(amount)}**"
            )
        )

    @commands.command(
        help="Takes a random amount of $ from someone", alises=["mug", "steal"]
    )
    async def rob(self, ctx, user: discord.Member):
        ...
        if target_cash == 0:
            return await ctx.send(
                embed=ctx.embed(
                    description="That user has no cash. Shame on you for trying to rob them."
                )
            )
        ...
        await ctx.send(
            embed=ctx.embed(
                description=f"You stole **${humanize.intcomma(amount)}** from {user.mention}!"
            )
        )
    ...
    async def work(self, ctx):
        ...
        embed = ctx.embed(
            description=f"In 30 seconds, type this backwards: \n`{word}`\nType `cancel` to cancel."
        )
        ...
        try:
            ...
            embed.description = (
                f"You didn't respond in time, the answer was `{correct_word}`"
            )
            ...
            if content == "cancel":
                ...
                embed.description = (
                    f"That's not the right word! The answer was `{correct_word}`"
                )
                await message.delete()
                return await ctx.send(embed=embed)
    ...
    async def daily(self, ctx):
        ...
        await ctx.send(
            embed=ctx.embed(
                description=f"You've collected **${cash}** from the daily gift!"
            )
        )
    ...
    async def fish(self, ctx):
        ...
        def terms(reaction, user):
            return (
                ...
                and str(reaction.emoji) in valid_fish
            )

        try:
            reaction, _ = await self.bot.wait_for(
                "reaction_add", timeout=10, check=terms
            )
        except asyncio.TimeoutError:
    ...
    async def beg(self, ctx):
        ...
        if random.randint(1, 3) == 1:
            return await ctx.send(
                "You sit all day on the street, but collect no money."
            )
    ...
    @commands.command(
        help="Resets a cooldown on one command", aliases=["reset", "cooldownreset"]
    )
    ...
    async def resetcooldown(self, ctx, command):
        ...
        if command == "daily":
            return await ctx.send(
                embed=ctx.embed(
                    description="You can't reset the daily command, sorry. "
        ...
        if cash < 400:
            return ctx.send(
                embed=ctx.embed(description="You need at least 400 dollars.")
            )
    ...
    async def bank_rob(self, ctx):
        ...
        """
        numbers = [
            "<:better1:826124826493190175>",
            "<:better2:826124826456227870>",
            "<:better3:826124826401177640>",
        ...
        for i in range(4):
            try:
                ...
                return await msg.edit(
                    content="You didn't pick fast enough.", delete_after=15
                )
            else:
                var += str(reaction.emoji)
                await msg.edit(content=f"{text}\n{var}")

        final = await ctx.send(
            "<a:loading:747680523459231834> Attempting to enter the vault...",
            delete_after=15,
        )
        ...
        if var != result:
            return await final.edit(
                content="❌ The patterns do not match and the vault door stays shut."
            )
        ...
        while True:
            ...
            try:
                ...
                await ctx.send(
                    f"You didn't respond in time, so I picked {choice} for you.",
                    delete_after=15,
                )
            ...
            if choice == "stay":
                ...
                await ctx.send(
                    f"💰 You grab another **${grabbed_amount}** to add to your moneybag.",
                    delete_after=15,
                )
            ...
            if choice == "leave":
                if amount == 0:
                    return await ctx.send(
                        ":( You have no money in your moneybag, I guess that's sad."
                    )
                break
    ...
    async def cooldowns(self, ctx):
        ...
        desc = "\n".join(
            f"{name}: {on_cooldown}" for name, on_cooldown in eco_commands.items()
        )
