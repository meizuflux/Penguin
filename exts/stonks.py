...
class Stocks(commands.Cog, command_attrs=dict(hidden=False)):
    ...
    async def buy(self, ctx, ticker: str = "MSFT", amount="1"):
        ...
        if total > cash:
            return await ctx.send(
                f"You need **${humanize.intcomma(total - cash)}** more in order to purchase"
        ...
        if answer:
            ...
            await self.bot.db.execute(
                stock_sql, ctx.guild.id, ctx.author.id, ticker, amount
            )
    ...
    async def sell(self, ctx, ticker: str = "MSFT", amount="1"):
        ...
        sql = "SELECT amount FROM stocks WHERE user_id = $1 AND guild_id = $2 AND ticker = $3"
        ...
        try:
            if amount != "max" and int(amount) > check:
                return await ctx.send(
                    f"You only have {check} {ctx.plural('share(s)', check)} of {ticker}"
                )
        except ValueError:
            return await ctx.send("Invalid amount provided.")
    ...
    async def portfolio(self, ctx, user: discord.Member = None):
        ...
        if len(stuff) == 0:
            return await ctx.send(
                f"{user.mention} has no stocks",
                allowed_mentions=discord.AllowedMentions().none(),
            )
        table = tabulate.tabulate(
            (dict(thing) for thing in stuff if thing["amount"] != 0),
            headers="keys",
            tablefmt="github",
        )
    ...
    async def check(self, ctx, search):
        search = search.upper()
        async with self.bot.session.get(
            f"{FINNHUB_URL}/search?q={search}&token={self.finnhub}"
        ) as r:
            data: dict = await r.json()
