...
class Walrus(commands.Bot):
    ...
    def run(self, *args, **kwargs):
        ...
        try:
            pool_pg = self.loop.run_until_complete(
                asyncpg.create_pool(dsn=self.settings["tokens"]["dsn"])
            )
    ...
    async def prep(self):
        ...
        for guild in self.guilds:
            await self.db.execute(
                "INSERT INTO guilds VALUES ($1) ON CONFLICT (guild_id) DO NOTHING",
                guild.id,
            )
        ...
        self.blacklist = dict(
            await self.db.fetch("SELECT user_id, reason FROM blacklist")
        )
    ...
    def get_subcommands(self, command):
        ...
        for command in command.commands:
            ...
            gotten_subcommands.extend(
                [f"{command} {alias}" for alias in command.aliases]
            )
            if isinstance(command, commands.Group):
