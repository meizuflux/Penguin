...
class Utilities(commands.Cog):
    ...
    async def password(self, ctx, length=16):
        ...
        password = "".join(temp)
        embed = discord.Embed(description=f'{length} digit random password: `{password.replace('`', random.choice(digits)))}`', color=self.bot.embed_color, timestamp=ctx.message.created_at)
